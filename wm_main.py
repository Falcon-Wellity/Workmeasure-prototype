...
if app_mode == "新規姿勢計測":
    ...
    # メイン表示エリアの分割（2:1の比率で左に映像、右にデータ・入力欄を配置）
            ...
            if st.button("💾 レポート内容を確定（プロトタイプ版保存）", type="primary"):
                try:
                    saved_id = save_report_to_csv(
                        ...
                        image_bgr=st.session_state.captured_image # 画像データを追加
                    )
...
# ==========================================
    ...
    if not os.path.isfile(CSV_FILE_PATH):
        ...
        try:
            ...
            # 最新のレポートが上にくるように逆順にする
            df = df.iloc[::-1]
            
            # セレクトボックス用に「ID + 日時 + 環境」の選択肢を作成
...
・作業高さ: {report_data['height_work']} cm"""
                ...
                # 【バグ＆改行対策】st.text_areaを読み取り専用にして、keyを紐づけることで強制更新
                st.text_area(
                    label="計測値一覧", 
                    value=metrics_text, 
                    height=240, 
                    disabled=True,                # ユーザーが編集できないように固定
                    label_visibility="collapsed", # 上部の余計なラベルを非表示に
            ...
            # keyに report_data['report_id'] を含めることで中身を強制更新
            st.text_area(
                "■ 理学療法士の評価（過去の記録）", 
